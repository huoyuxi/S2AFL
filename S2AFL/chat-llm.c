...
char *chat_with_llm(char *prompt, char *model, int tries, float temperature)
{
    ...
    if (strcmp(model, "instruct") == 0)
    {
        asprintf(&data, "{\"model\": \"hf-mirror.com/bartowski/Qwen2.5-72B-Instruct-GGUF:latest\", \"prompt\": \"%s\", \"max_tokens\": %d, \"temperature\": %f}", prompt, MAX_TOKENS, temperature);
    }
    else
    {
        asprintf(&data, "{\"model\": \"hf-mirror.com/bartowski/Qwen2.5-Coder-32B-Instruct-GGUF:latest\", \"prompt\": \"%s\", \"max_tokens\": %d, \"temperature\": %f}", prompt, MAX_TOKENS, temperature);
    }
