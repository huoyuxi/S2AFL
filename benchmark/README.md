...
The execution helpers also accept optional environment overrides for container-specific layouts:

```bash
export PFBENCH_CONTAINER_HOME=/path/inside/container/home
export PFBENCH_CONTAINER_WORKDIR=/path/inside/container/workdir
export PFBENCH_FUZZER_ROOT=/path/inside/container/fuzzer-root
export PFBENCH_GCOVR_HELPER=/path/inside/container/gcovr.py
```

If unset, the scripts keep the original ProFuzzBench-compatible defaults.
...
## Step-2. Run fuzzing
Run [profuzzbench_exec_common.sh script](scripts/execution/profuzzbench_exec_common.sh) to start an experiment. The script takes 8 required arguments and 1 optional cleanup flag as listed below.
...
- ***4th argument (FUZZER)***   : fuzzer name (e.g., aflnet) -- this name must match the fuzzer folder inside the container worktree; by default the helper resolves `${PFBENCH_FUZZER_ROOT}/<FUZZER>` after deriving container defaults from `$HOME`
...
- ***9th argument (DELETE)***   : optional non-empty flag to remove finished containers after results are copied out
...
ProFuzzBench also includes scripts for running all fuzzers on all targes, with pre-configured parameters. In this public S2AFL release, the retained benchmark container automation is limited to the AFLNet path that is still shipped in the repository. The helper scripts [profuzzbench_build_all.sh](scripts/execution/profuzzbench_build_all.sh) and [profuzzbench_exec_all.sh](scripts/execution/profuzzbench_exec_all.sh) therefore build and run the `aflnet` container workflow only.
