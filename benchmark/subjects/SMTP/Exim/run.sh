...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "llm") ; then
  ...
  timeout -k 2s --preserve-status $TIMEOUT ${FUZZER_BIN} -d -i ${INPUTS} -x ${WORKDIR}/smtp.dict -o $OUTDIR -N tcp://127.0.0.1/25 $OPTIONS -c ${WORKDIR}/clean exim -bd -d -oX 25 -oP /var/lock/exim.pid
